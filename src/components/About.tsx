        ...
        >
          ...
          <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-3 mb-4 sm:mb-6">
            {["⚙️ Linux Specialist", "🛠️ DevOps Engineer", "☁️ Cloud Explorer", "🤖 AI Innovator"].map((tag) => (
              <span key={tag} className="px-3 py-1.5 sm:px-4 sm:py-2 text-xs sm:text-sm rounded-full bg-primary/10 text-primary border border-primary/20 font-medium">
                {tag}
              </span>
            ))}
          </div>
          ...
          {highlights.map((item, i) => (
            ...
            >
              <div className="w-10 h-10 sm:w-14 sm:h-14 rounded-xl sm:rounded-2xl bg-primary/10 flex items-center justify-center mx-auto mb-3 sm:mb-4 group-hover:bg-primary/20 transition-colors">
                <item.icon size={20} className="text-primary sm:hidden" />
                <item.icon size={24} className="text-primary hidden sm:block" />
              </div>
